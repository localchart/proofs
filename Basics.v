...
(** wy: We can use destruct here because 
...
So we are not going into any recursion -- (n' + 1) will not be
evaluated.
...
(** wy: so the proof script will be highly dependent on the order of goals. This is not resilient. *)
...
(** wy: after destructing b, the context ((negb (negb [])) = []) is duplicated. *)
...
(** wy: then why do we left the order of goals at the hands of Coq? *)
...
(* wy: Is it better to intro H separately in each subgoals?
Proof.
  intros b c H.
  destruct b.
  Case "b = true".
    reflexivity.
  Case "b = false".
    (* the hypothesis H will be changed in this case, but not shown
       until we get to this subgoal. *)
...
(* wy: best way: destruct c first, and intro H only in second subgoal *)
Proof.
  intros b c.
  destruct c.
  Case "c = true".
...
(* wy: if destruct b first, then "c = true" will be duplicated, we
   need to rewrite twice.
...
(* wy: if intro H first, H will be duplicated in both cases when we
   destruct c, but will not be shown in the context. This is
   confusing. There wouldn't be this problem with natural deduction.
...
(* wy: if don't intro H, then intro H will need to be written twice.
Proof.
  intros b c.
  destruct b.
  intro H1.
...
(* wy: induction on n because * induction on n *)
...
(* wy: induction on n first because "+" inducts on n.
   sounds like this lemma is necessary for proving commutativity! *)
...
(* wy: best proof - no lemmas, using occurrence_clause to accurately
   locate occurrencies of induction *)
...
Proof.
  ...
  Case "n = 0".
   ...
   SCase "m = 0".
    simpl. reflexivity.
   SCase "m = S m'".
    simpl. rewrite <- IHm'. reflexivity. (* why rewrite <-? *)
  Case "n = S n'".
    simpl. rewrite -> IHn'.
    induction m as [|m'] in |- *.
    SCase "m = 0".
...
(* wy: alternative proof 1 - no lemmas, using clear to get rid of IHs
   that are no l onger useful, thus avoiding duplicate them after
   induction.
...
Proof.
  ...
  Case "n = S n'".
    simpl. rewrite -> IHn'.
    clear IHn'.                 (* get rid of IHn' after rewriting *)
    induction m as [|m'].
    SCase "m = 0".
     simpl. reflexivity.
    SCase "m = S m'".
...
(* wy: alternative proof 2 - without plus_n_Sm, using elim
Proof.
  intros n m.
  induction n as [|n']. simpl.
  Case "n = 0".
...
(* wy: alternative proof 3 - using assert
...
(* wy: standard proof - using plus_n_Sm as lemma. Not preferred,
   because plus_n_Sm is not general enough to be used in a good number
   of places.
...
(* wy: using accurate locators for induction variable can avoid using lemmas.
   Here induction is used only on the conclusion "|- *" at occurrences 1 and 3.
   induction n' as [|n''] in |- * at 1 3. *)
Proof.
  intros n. 
  induction n as [|n'].
  Case "n = 0".
...
(* wy: alternatively, you can use a separate lemma (plus_n_Sm). But I
   prefer not to use it because the lemma says something not very
   meaningful or general.
...
(* wy: is it really so hard to target (n + m) ? *)
...
Proof.
  ...
  Case "m = S m'".
   ...
   SCase "n = S n'".
    simpl. 
    rewrite <- IHn'.
    rewrite -> plus_swap.   (* use plus_swap *)
    reflexivity.
  Qed.
...
(* wy: using change, also need destruct (evenb (S n')) 
   It feels like proof by cases on (evenb (S n')).
*)
Proof.
  intros.
  induction n as [|n'].
...
(* wy: using setoid_rewrite - but still need destruct *)
Proof.
  induction n.
  reflexivity.
  setoid_rewrite IHn.
  destruct (evenb (S n)). reflexivity. reflexivity.
Qed.
...
ble_nat_refl = 
fun n : nat =>
nat_ind (fun n0 : nat => true = ble_nat n0 n0)
  (let Case := "n = 0" in eq_refl)
  (fun (n' : nat) (IHn' : true = ble_nat n' n') =>
   let Case := "n = S n'" in
   eq_ind_r (fun b : bool => b = ble_nat n' n') eq_refl IHn') n
...
(* wy: interesting, this one doesn't seem to need induction but actually need *)
...
(* c *)
Theorem inc_bin2nat_comm : forall n:bin,
  bin2nat(inc n) = S(bin2nat n).
Proof.
  intros.
...
Eval simpl in (bin2nat (nat2bin 21)).



Theorem bin2nat_back : forall n:nat,
  (bin2nat (nat2bin n)) = n.
Proof.
  intros.
  induction n as [|n'].
  ...
  induction n' as [|n''].
  reflexivity.
  change (bin2nat (nat2bin (S (S n'')))) with (S (S (bin2nat (nat2bin n'')))).
  rewrite -> IHn''.
  reflexivity.
Qed.
...
Theorem back_normal : forall n:bin,
  nat2bin(bin2nat n) = normalize n.
Proof.
  intros.
  reflexivity.
Qed.

Eval compute in (normalize (U (T (T (U Z))))).
